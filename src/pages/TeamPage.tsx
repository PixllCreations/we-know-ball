...
const TeamPage = () => {
  ...
  const rosterQ = useQuery({ queryKey: ["roster", id], queryFn: () => getTeamRoster(id), enabled: !!id });
  const scheduleQ = useQuery({ queryKey: ["schedule", id], queryFn: () => getTeamSchedule(id), enabled: !!id });
  ...
  return (
    <div className="space-y-10">
      <Link to="/teams" className="font-mono text-xs uppercase tracking-wider text-muted-foreground hover:text-primary">
        ← All teams
      </Link>
      ...
      >
        <div className="flex flex-col items-start gap-6 md:flex-row md:items-center">
          {team.logos?.[0]?.href && (
            <img src={team.logos[0].href} alt={`${team.displayName} logo`} className="h-32 w-32 object-contain" />
          )}
          <div>
      ...
      {stats.length > 0 && (
        <section className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-6">
          {stats.slice(0, 6).map((s) => (
            <div key={s.name} className="rounded-lg border border-border bg-gradient-stat p-4">
              ...
              <table className="w-full text-sm tabular">
                ...
                <tbody>
                  {rosterQ.data?.athletes?.map((p) => (
                    <tr key={p.id} className="border-b border-border/60 last:border-0 hover:bg-surface-hover">
                      <td className="px-4 py-2 font-mono text-xs text-muted-foreground">{p.jersey ?? "—"}</td>
                      <td className="px-4 py-2">
                        <div className="flex items-center gap-2.5">
                          {p.headshot?.href ? (
                            <img src={p.headshot.href} alt="" className="h-8 w-8 rounded-full object-cover bg-muted" loading="lazy" />
                          ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      {opp?.team.logo && (
                        <img src={opp.team.logo} alt="" className="h-8 w-8 object-contain" loading="lazy" />
                      )}
                    ...
                    {state === "post" ? (
                      <div className={cn("text-right tabular text-sm font-semibold", won ? "text-success" : "text-muted-foreground")}>
                        <div>{won ? "W" : "L"}</div>
                        <div className="font-mono text-[10px]">{me?.score}-{opp?.score}</div>
                      </div>
                    ) : (
